...
class Agent():
    def __init__(self, envs, world, stage, action_type, num_envs, state_dim, action_dim, save_dir, save_model_step,
                 ...
                 target_models, predict_model, gamma, gamma_int, learning_rate, entropy_coef, V_coef, max_grad_norm,
                 clip_param, batch_size, num_epoch, is_normalize_advantage, V_loss_type, target_kl, gae_lambda, int_adv_coef,
                 ext_adv_coef, num_target_model, alpha_DRND1, alpha_DRND2, update_proportion, additional_bonus_state_8_4_option, device):
        self.world = world
        self.stage = stage
        ...
        self.target_models = [target_model.to(self.device) for target_model in target_models]
        ...
        self.num_target_model = num_target_model
        self.alpha_DRND1 = alpha_DRND1
        self.alpha_DRND2 = alpha_DRND2
        ...
        self.DRND_loss = np.zeros((1000, )).reshape(-1)
    ...
    def update_loss_statis(self, loss_p, loss_v, loss_v_int, loss_e, loss, approx_kl_div, loss_DRND):
        ...
        self.DRND_loss[self.loss_index] = loss_DRND
    ...
    def compure_reward_int(self, next_state):
        ...
        with torch.no_grad():
            ...
            for target_model in self.target_models:
                target_features = target_model(next_state)
                list_target_features.append(target_features)
            list_target_features = torch.stack(list_target_features, 0)
            mean_target_features = list_target_features.mean(0)
            B2_target_features = (list_target_features**2).mean(0)
            features = self.predict_model(next_state)
        rewards_int = (self.alpha_DRND1 * ((features - mean_target_features)**2).mean(-1)
                        + self.alpha_DRND2 * torch.sqrt(torch.clamp((features**2 - mean_target_features**2).abs() / 
                                                                    (B2_target_features - mean_target_features**2 + 1e-9), 1e-3, 1)).mean(-1))
        return rewards_int.reshape(-1)

    def compure_target_features(self, next_state):
        next_state = torch.tensor(np.array(next_state), device = self.device)
        next_state = next_state[:, 3, :, :].reshape(-1, 1, next_state.shape[2], next_state.shape[3])
        next_state = ((next_state - self.obs_rms.mean.to(self.device)) / torch.sqrt(self.obs_rms.var.to(self.device))).clip(-5, 5)
        with torch.no_grad():
    ...
    def learn(self):
        ...
        rewards_int = []
        with torch.no_grad():
            for next_state in np.transpose(np.array(next_states_), axes = [1, 0, 2, 3, 4]):
        ...
        with torch.no_grad():
            for next_state in np.transpose(np.array(next_states_), axes = [1, 0, 2, 3, 4]):
                target_feature = self.compure_target_features(next_state)
                target_features.append(target_feature)
        target_features = torch.stack(target_features, 1)
        target_features = target_features.reshape((target_features.shape[0], -1, target_features.shape[-1])) 
        ...
        for state, next_state, reward, reward_int, done, value, value_int in zip(states[::-1], next_states_[::-1], rewards[::-1], rewards_int[::-1], dones[::-1], old_values[::-1], old_values_int[::-1]):
            ...
            # reward /= torch.sqrt(self.ext_reward_rms.var.to(self.device) + 1e-9)
        ...
        for epoch in range(self.num_epoch):
            ...
            for i in range(len(old_values)//self.batch_size):
                ...
                batch_target_features = target_features[batch_ids]
                index = torch.arange(0, len(batch_target_features), device = self.device)
                random_model = torch.randint(0, self.num_target_model, (len(batch_target_features),)).reshape(-1)
                batch_target_features = batch_target_features[index, random_model]
                
                features = self.predict_model(batch_next_states)

                mask = torch.rand(len(features)).to(self.device)
                mask = (mask < self.update_proportion).type(torch.FloatTensor).to(self.device)
                loss_DRND = ((features - batch_target_features)**2).mean(-1)
                loss_DRND = (loss_DRND * mask).sum() / torch.max(mask.sum(), torch.Tensor([1]).to(self.device))

                # update model
                loss = loss_V * self.V_coef + loss_V_int * self.V_coef + loss_P - entropy * self.entropy_coef

                self.update_loss_statis(loss_P.item(), loss_V.item(), loss_V_int.item(), entropy.item(), loss.item(), approx_kl_div.item(), loss_DRND.item())
